...
bool Sphere::Set(ID3D11Device* pDevice, float radius, int slicecount, int stackcount)
{
	...
	PC_VERTEX topVertex = { D3DXVECTOR3(0.0f, radius, 0.0f), D3DXVECTOR4(DirectX::Colors::Black) };
	PC_VERTEX botVertex = { D3DXVECTOR3(0.0f, -radius, 0.0f), D3DXVECTOR4(DirectX::Colors::Black) };
	...
	for (int i = 0; i < stackcount - 1; ++i)
	{
		...
		for (int j = 0; j <= slicecount; ++j)
		{
			...
			v.p = { x,y,z };
			v.c = D3DXVECTOR4(DirectX::Colors::Black);
