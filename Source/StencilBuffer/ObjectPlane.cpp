...
bool ObjectPlane::PreRender(ID3D11DeviceContext* pContext)
{
	pContext->UpdateSubresource(m_DxObject.m_pConstantBuffer.Get(), 0, nullptr, &m_cbData, 0, 0);
	return m_DxObject.PreRender(pContext, sizeof(PC_VERTEX));
}
...
bool ObjectPlane::PostRender(ID3D11DeviceContext* pContext)
{
	return m_DxObject.PostRender(pContext, sizeof(PC_VERTEX), m_iIndex);
}
